...
- EMF から復元した文字は検索・コピーできます。`--font` を省略すると標準の日本語CIDフォントを参照するため、フォント本体はPDFに入りません。配布先のフォント環境に依存しない PDF が必要な場合は、使用グリフだけを埋め込む `--font` を指定してください。
...
- プリンタドライバ由来の版面層は、LHA `-lh5-` を展開して EMF または WMF として解釈します。文字、JPEG の配置、埋め込み DIB（1/4/8/24 bpp、RLE4/RLE8）、`PATCOPY` の塗り、private のパス（クリップ・塗り・縁取り・ベジェ）を再現します。
